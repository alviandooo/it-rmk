    ...
    <div class="page-content">
        <div class="row">
            <div class="card">
                <div class="card-body">
                    <h2>Data Item</h2>
                    <p>{{Breadcrumbs::render('kategori-peripheral')}}</p>
                </div>
            </div>
        </div>
            ...
            <div class="card">
                <div class="card-body">
                    <div class="table-responsive">
                        ...
                        <a href="{{route('item.exportitembykategori',['5','1'])}}" class="btn btn-sm btn-warning" style="margin-bottom: 15px" target="_blank">Excel</a>
                        <a href="{{route('item.exportitembykategori',['5','2'])}}" class="btn btn-sm btn-danger" style="margin-bottom: 15px" target="_blank">PDF</a>
                        {{-- <button class="btn btn-sm btn-primary" style="margin-bottom: 15px" id="btn-tambah-item">Tambah</button> --}}
    ...
    <script>
        $(document).ready(function () {
            var dti = $('#dtitem').DataTable({
                ...
                "serverSide": true,
                "order": [[ 7, "desc" ]],
                "ajax": "{{route('item.dataperipheral')}}",
                "columns": [
                ...
                $.ajax({
                    ...
                    success: function(response) {
                        if(response.status == 201){
                            ...
                            var url = "{{url('/admin/item/peripheral')}}";
                            window.location = url;
